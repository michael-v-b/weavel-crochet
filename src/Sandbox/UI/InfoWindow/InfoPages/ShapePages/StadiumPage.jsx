...
const StadiumPage = ({ objects }) => {
  ...
  const handleWidth = (newWidth) => {
    objects.forEach((object) => {
      ...
      const temp = isHalf
        ? Math.max(height, newWidth / 2)
        : Math.max(height, newWidth);
  ...
  const handleHeight = (newHeight) => {
    objects.forEach((object) => {
      ...
      let temp = isHalf
        ? Math.min(width, newHeight * 2)
        : Math.min(width, newHeight);
  ...
  return (
    <>
      ...
      <WidthField ref={widthRef} objects={objects} getWidth={handleWidth} />
      <HalfField objects={objects} getHalf={handleHalf} />
    </>
  );
};
